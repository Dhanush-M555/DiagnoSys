    ...
    def _analyze_metrics_with_rag(self, metrics: Dict[str, float], log_lines: List[str], timestamp: str) -> List[Dict[str, Any]]:
        ...
        for pattern, score in similar_patterns:
            ...
            if pattern_type == "saturation_induced_latency":
                ...
                # Debug output
                print(f"Replication check for {timestamp}: high_time_taken={high_time_taken_count}, replication_issues={replication_issues}, sync_delays={sync_delays}")
                ...
                if (high_time_taken_count >= 1 and 
                    ...
                    details = (
                        ...
                        f"- High TimeTaken replication events: {high_time_taken_count}\n"
                        f"- Replication errors: {replication_issues}\n"
                        f"- Sync delays/timeouts: {sync_delays}\n"
                        f"Semantic similarity score: {1.0 - score:.2f}\n"
                        f"Metric alignment score: {metric_alignment:.2f}\n"
    ...
    def format_fault_report(self, analysis_result: Dict[str, Any]) -> str:
        ...
        if analysis_result.get("faults"):
            ...
            for i, fault in enumerate(analysis_result["faults"], 1):
                ...
                formatted_report.append(f"Confidence: {fault['confidence']:.2%}")
                formatted_report.append("")
                
                # Details
                formatted_report.append("DETAILS:")
                for line in fault['details'].split('\n'):
                    formatted_report.append(f"  {line}")
                formatted_report.append("")
                ...
                # Related metrics
                formatted_report.append("RELATED METRICS:")
                for metric, value in fault['related_metrics'].items():
